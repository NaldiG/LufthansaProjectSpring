...
import com.naldi.naldiProject.payload.request.LoginRequest;
import com.naldi.naldiProject.payload.request.SignupRequest;
import com.naldi.naldiProject.payload.response.JwtResponse;
import com.naldi.naldiProject.payload.response.MessageResponse;
...
public class AuthController {
	...
	public ResponseEntity<?> resetPassword(@Valid @RequestBody SignupRequest signUpRequest) {
		if (userRepository.existsByUsername(signUpRequest.getUsername())) {
			...
			if (userRepository.existsByEmail(signUpRequest.getEmail())) {
				Optional<User> user= userRepository.findByEmailAndUsername(signUpRequest.getEmail(), signUpRequest.getUsername());
				user.ifPresent(value -> value.setPassword(encoder.encode(signUpRequest.getPassword())));
